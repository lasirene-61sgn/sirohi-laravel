...
<div class="p-6 md:p-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Edit Helpline</h2>
        </div>

        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.helpline.update', $helpline->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Name</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $helpline->name) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="heading_name" id="heading_name" value="{{ old('heading_name', $helpline->heading_name) }}" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                ...
                <div class="field-group" data-name="{{ $name }}">
                    ...
                    <div class="inputs-container space-y-2">
                        @php $values = $helpline->$name ?? []; @endphp
                        @forelse($values as $index => $value)
                        <div class="flex space-x-2">
                            <input type="{{ $name == 'emails' ? 'email' : 'text' }}" name="{{ $name }}[]" value="{{ $value }}"
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end space-x-3">
                    ...
                    <button type="submit" 
                            class="px-6 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 shadow-md transition">Update Helpline</button>
                </div>
            </form>
        </div>
    </div>
